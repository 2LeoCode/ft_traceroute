...
static void TR_missingOptionArg(const char *option, int idx, char *usage) {
  fprintf(stderr, "Option `%s' (argc %d) requires an argument: `%s'\n", option,
          idx, usage);
  exit(TR_FAILURE);
}
...
static void TR_invalidArg(const char *name, const char *arg, size_t pos,
                          int index) {
  fprintf(stderr,
...
static void TR_invalidOptionArg(const char *option, const char *arg,
                                int index) {
...
TR_Options TR_parseArgs(int argc, const char *const *argv) {
  size_t arg_cnt = 0;
  TR_Options options = {.global_opts = argc == 1 ? TR_HELP : 0,
                        .method = TR_UDP,
