...
int TR_initSockets(TR_SocketSet *tr, const TR_Options *options,
  ...
  for (uint8_t i = 0; i < tr->nfds; ++i) {
    tr->sockets[i] = (TR_Socket){
        .fileno = socket(driver->domain, driver->type | SOCK_NONBLOCK,
                         driver->protocol),
    };
    if (tr->sockets[i].fileno == -1)
      return TR_FAILURE;

    if (setsockopt(tr->sockets[i].fileno, SOL_IP, IP_TTL,
                   (int[]){i + options->firstTtl}, sizeof(int)) ||
        setsockopt(tr->sockets[i].fileno, SOL_IP, IP_RECVERR, (int[]){1},
                   sizeof(int)))
      return TR_FAILURE;
